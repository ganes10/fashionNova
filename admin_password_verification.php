...
// Check if the admin OTP verification form is submitted
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $storedOTP = isset($_SESSION['admin_reset_otp']) ? $_SESSION['admin_reset_otp'] : null;
    ...
    if ($enteredOTP == $storedOTP) {
        ...
        // Redirect to the reset password form for admin
        header("location:admin_reset_password.php");
        exit();
    } else {
        $_SESSION['message'] = "Invalid OTP. Please try again.";
        header("location:admin_password_verification.php");  // Redirect back to OTP verification
        exit();
    }
}
?>
...
<!-- Display the OTP verification form for admin -->
<form action="<?php echo htmlspecialchars('admin_password_verification.php') ?>" method="POST" novalidate>
